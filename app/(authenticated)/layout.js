...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-[#fafafa] font-[family-name:var(--font-poppins)] text-gray-900 overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`${isCollapsed ? "w-20" : "w-64"} bg-white border-r border-gray-100 hidden md:flex flex-col transition-all duration-300 relative z-30`}
      >
        {/* Premium Floating Toggle */}
        <button
          onClick={toggleSidebar}
          className={`
            absolute -right-4 top-10 h-8 w-8 rounded-full 
            bg-white/80 backdrop-blur-md border border-gray-100
            flex items-center justify-center shadow-[0_8px_30px_rgb(0,0,0,0.06)]
            hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] hover:scale-110 
            transition-all duration-300 z-50 group border-l-0
          `}
          style={{ borderColor: `${theme.primaryColor}20` }}
        >
          <div
            className="absolute inset-0 rounded-full opacity-10 group-hover:opacity-20 transition-opacity"
            style={{ backgroundColor: theme.primaryColor }}
          ></div>
          <svg
            className={`w-4 h-4 transition-transform duration-500 ease-out ${isCollapsed ? "rotate-180" : ""}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div className="p-6 h-20 flex items-center border-b border-gray-100 overflow-hidden">
          <div className={`flex items-center transition-all ${isCollapsed ? "justify-center w-full" : "justify-center w-full px-2"}`}>
            <div className="h-10 w-full max-w-[120px] shrink-0 flex items-center justify-center">
              <img src={theme.logo} alt="Logo" className="h-full w-auto object-contain scale-125" />
            </div>
          </div>
        </div>

        {/* Ultra-Simplified Navigation */}
        <nav className="flex-1 px-4 space-y-1 mt-8">
          {navigation.filter(item => (item.name !== "Create IP") || user?.role === "super_admin").map((item) => {
            const isActive = pathname === item.href;
            return (
              <button
                key={item.name}
                onClick={() => router.push(item.href)}
                className={`w-full flex items-center px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 group relative ${isActive
                  ? "text-white shadow-lg shadow-black/5"
                  : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
                  }`}
                style={isActive ? { backgroundColor: theme.primaryColor } : {}}
              >
                <div className={`${isActive ? "text-white" : "text-gray-400 group-hover:text-gray-900"}`}>
                  {item.icon}
                </div>
                ...
                {isCollapsed && (
                  <div
                    className="absolute left-full ml-4 px-3 py-2 text-white text-[10px] rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-[100] whitespace-nowrap font-semibold uppercase tracking-widest shadow-xl"
                    style={{ backgroundColor: theme.primaryColor }}
                  >
                    {item.name}
                  </div>
        ...
        <div className="p-4 mt-auto border-t border-gray-100">
          <button
            ...
            className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'justify-center'} px-4 py-3 text-sm font-semibold text-gray-400 rounded-xl hover:bg-red-50 hover:text-red-500 transition-all group overflow-hidden`}
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-10 z-20 shrink-0">
            ...
            {user?.role !== "super_admin" ? (
              <div className="flex items-center space-x-4 group">
                ...
                <div className="flex flex-col">
                  <div
                    className={`flex items-center space-x-2 ${user?.ips?.length > 1 ? 'cursor-pointer select-none group' : ''}`}
                    onClick={() => user?.ips?.length > 1 && setIsSelectorOpen(!isSelectorOpen)}
                  >
                    <h2 className="text-[15px] font-bold text-gray-950 tracking-[0.05em] uppercase opacity-95 leading-none transition-colors group-hover:text-gray-600">
                      {activeIp?.name || "Unknown IP"}
                    </h2>
                    {user?.ips?.length > 1 && (
                      <div className="relative flex items-center justify-center">
                        <div className="h-5 w-5 rounded-md hover:bg-gray-50 flex items-center justify-center transition-all">
                          <svg className={`w-3 h-3 text-gray-400 transition-transform duration-300 ${isSelectorOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {isSelectorOpen && (
                          <div className="absolute top-8 left-0 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-[100] animate-in fade-in zoom-in-95 duration-200">
                            <div className="px-4 py-2 mb-1 border-b border-gray-50">
                              <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest">Switch Property</p>
                            </div>
                            {user.ips.map((ip) => (
                              <button
                                key={ip.id}
                              ...
                              >
                                <div className="h-7 w-7 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-center mr-3 text-[11px] font-bold text-gray-400">
                                  {ip.code.substring(0, 2)}
                                </div>
                                <div className="flex flex-col">
                                ...
                                {activeIp?.id === ip.id && (
                                  <div className="ml-auto h-2 w-2 rounded-full" style={{ backgroundColor: theme.primaryColor }}></div>
                                )}
                              </button>
                            ))}
                          </div>
                  ...
                  <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                    {user?.role === "super_admin" ? "Global System Console" : "Property Manager"}
                  </span>
                </div>
              </div>
            ) : (
          ...
          <div className="flex items-center space-x-6">
            {/* Switch to Super Admin button - only when entered as manager */}
            {user?.role === "super_admin" && enteredAsManager && (
              <button
            ...
            <div className="flex flex-col items-end mr-1">
              <span className="text-[13px] font-bold text-gray-950 uppercase tracking-tight leading-none">
                {user?.username || "Admin"}
              </span>
              <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-[0.1em] mt-1">
                {user?.role === "super_admin" ? "Role: Super Admin" : "Role: IP Admin"}
              </span>
            </div>
            <div className="h-10 w-10 rounded-full border border-gray-100 bg-gray-50 flex items-center justify-center p-1 cursor-pointer hover:bg-gray-100 transition-all shadow-sm group">
              <div
                className="h-full w-full rounded-full flex items-center justify-center text-[10px] font-bold text-white transition-all"
                style={{ backgroundColor: theme.primaryColor }}
              >
