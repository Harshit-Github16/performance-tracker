...
const DATA_TYPES = ["integer", "decimal", "string", "boolean", "jsonb"];
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-gray-950/20 backdrop-blur-[20px] animate-in fade-in duration-200">
                    <div ref={modalRef} className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-gray-100/50 overflow-hidden">
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            {/* Sports with Checkboxes */}
                            <div className="flex flex-col space-y-2">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Sports</label>
                                <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 space-y-2 max-h-48 overflow-y-auto">
                                    {sports.length === 0 ? (
                                        <p className="text-xs text-gray-400">Loading sports...</p>
                                    ) : (
                                        sports.map((sport) => {
                                            ...
                                            return (
                                                <label
                                                    key={sport.id}
                                                    className="flex items-center gap-3 cursor-pointer group hover:bg-white px-3 py-2 rounded-lg transition-all"
                                                >
                                                    <div
                                                        onClick={() => toggleSport(sport.id)}
                                                        className={`h-5 w-5 rounded border-2 flex items-center justify-center transition-all cursor-pointer ${isSelected ? "border-gray-950 bg-gray-950" : "border-gray-200 group-hover:border-gray-400"}`}
                                                    >
                                                        {isSelected && (
